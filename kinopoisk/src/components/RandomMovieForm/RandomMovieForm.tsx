...
import { IMovieFilters } from "~components/RandomMovie";
import { RandomMovieSelect } from "~components/RandomMovieSelect";
import { RandomMovieSlider } from "~components/RandomMovieSlider";
...
interface IRandomMovieForm {
	handleMovieFetch: (value: IMovieFilters) => void
}

export function RandomMovieForm({ handleMovieFetch }: IRandomMovieForm) {
	...
	function handleInputChange(name: string, value: string | number | number[]) {
		if (name === "kpRating") {
